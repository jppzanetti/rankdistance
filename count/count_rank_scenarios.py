...
version generates a large list with every possible combination of path
solution sizes, using more memory.
"""
...
def generate_l(k_list):
    """Generates all possible lists of l values for the given path
    lengths."""

    # Return if no more ks
    if len(k_list) == 0:
        return [[]]

    # Possible ls for the last k
    possible_l = [[x] for x in range(min_l(k_list[-1]), k_list[-1] + 1)]

    # Compound with the other l values
    return [x + y for (x, y) in product(generate_l(k_list[:-1]), possible_l)]
...
def count_rank_scenarios(k_cycles, k_paths, sol_size = -1):
    total = 0

    l_paths = generate_l(k_paths)

    for l_list in l_paths:
...
if __name__ == '__main__':
    ...
    print(int(count_rank_scenarios(k_cycles, k_paths, args.m)))
